...
스케줄: Airflow 3 DAG (VM · LocalExecutor) 가 태스크 넷으로 쪼개 하루 8회 실행 · 회차 끝 Cloudflare Pages 자동 배포
        systemd 는 워치리스트 · 백업 · 유지보수 같은 부수 작업만 맡는다
```
...
| 스케줄 · 배포 | **Airflow 3 (LocalExecutor) + wrangler** | 회차 · 웨어하우스 적재를 태스크 넷 + 배포 훅으로 쪼개 하루 8회 (3시간 간격) 실행 ([마이그레이션](docs/MIGRATION.md)) · 회차 끝 Pages 직접 업로드. systemd 는 워치리스트 · 백업 · 유지보수 같은 부수 작업만 맡는다 |
