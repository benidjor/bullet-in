...
- **이종 소스 통합**: RSS · REST API · 정적 HTML · JS 렌더링 · X (트위터) · 한국 커뮤니티를 단일 어댑터 인터페이스로 추상화했습니다. 소스 특성에 맞는 도구를 각각 선택합니다 (정적 = httpx, API = Guardian Open Platform, X = 쿠키 주입 Playwright).
- **병렬 수집과 실패 격리**: asyncio 기반 팬아웃으로 소스를 동시에 호출하며, 한 소스가 실패해도 나머지 수집은 정상 진행됩니다.
- **공신력 스코어링**: Tier 0 (Arsenal.com 공식) 부터 4 (타블로이드) 까지를 YAML 로 외부화하고 confidence 값으로 정렬합니다. 기자 단위 tier 가 매체 tier 보다 우선 적용됩니다 (전담 기자의 기사만 등급 상향).
- **중복 제거와 변경 감지**: content_hash 와 URL 정규화에 DB UNIQUE 제약을 더해 애플리케이션과 DB 양쪽에서 중복을 막는 이중 방어입니다.
- **LLM 번역 · 요약**: Gemini 3.1 Flash-Lite 를 사용합니다.
  - 산출물: 제목 · 본문 번역, 1줄 · 3줄 요약, 영입 단계 분류입니다.
  - 멱등: 신규 행만 처리하므로 같은 실행을 다시 돌려도 결과가 달라지지 않습니다.
  - 429 (rate limit): 해당 실행의 번역을 중단하고 다음 실행이 남은 건을 이어서 처리합니다.
- **선수 추출과 이적 상태 관리**: 기사 본문에서 선수를 추출해 주체와 단순 언급으로 구분해 매핑하고 선수별 페이지와 이적 상태 (영입 진행 · 확정 · 무산 · 타 클럽행 · 방출) 를 명단에서 관리합니다.
- **데이터 품질 게이트**: 실행 마지막 단계의 dbt test 21종이 배포를 차단하는 장치입니다 (§6).
...
- **행동 로그와 레이크하우스**: GA4 이벤트를 Iceberg 에 Bronze · Silver · Gold 로 적재하고 (사용자 · 세션 · 코호트 테이블), 서빙 DB 의 변경 이력과 일별 스냅샷도 같은 레이크하우스에 보존합니다 (§9).

**수집 소스**: 활성 소스는 9종이며 고정 tier 7종과 항목별 동적 tier 2종으로 구성했습니다 (X 와 커뮤니티는 게시물에 언급된 기자 · 매체의 공신력으로 tier 를 산출합니다). 아래 표에는 2026-07 에 비활성으로 돌린 1종을 포함해 10종을 싣습니다. 언론 5종은 공통 이적 키워드 필터를 공유합니다 (`config/sources.yaml`).
...
- **dbt 게이트**: 실행의 `gate` 태스크가 `dbt build` 로 스테이징 5개와 Gold 3개를 생성하고 테스트 21종 (unique 5 · not_null 10 · accepted_values 4 · relationships 2) 을 돌립니다. 차단되면 `deploy_site` 가 실행되지 않고 알림이 발송됩니다. 임계 미만의 결측은 경고로 분류해 로그에만 남깁니다. 설계는 [dbt 품질 게이트 스펙](docs/superpowers/specs/2026-08-31-dbt-quality-gate-design.md) 에 있습니다.
...
- **번역 품질 게이트**: LLM 이 만든 본문을 LLM 없이 규칙 코드로 검사합니다.
  원문 숫자 누락 · 원문에 없던 수치 주입 · 인용 보존 실패 · 구단명 주입 · 인명 주입 · 원문 복제 잔존율 (글자 8-gram 기준 0.75 초과) 여섯 가지를 보고, 하나라도 걸리면 사유를 프롬프트에 붙여 최대 두 번까지 다시 생성한 뒤 세 시도 중 위반이 가장 적은 것을 채택합니다.
  게이트는 재생성 트리거이지 폐기 조건이 아니므로 본문을 버리지 않습니다. 잔존율이 임계를 넘은 채로 남은 기사는 수집 현황 대시보드에 올려 사람이 검토합니다.
...
- **테스트**: 테스트 1,792개를 유지합니다 (단위 · 통합 · DAG 임포트 · 브라우저 1건). 통합 테스트는 CI 의 MariaDB 컨테이너에 실제로 연결해 돌리는 방식입니다.
...
메달리온 아키텍처를 2개 운영합니다. 기사와 행동 로그는 원천 시스템도 저장소도 달라 층을 따로 구성했습니다.

```
기사 파이프라인, 메달리온 1

  Bronze   MongoDB   raw_items, 소스 원문을 변형 없이 보존
     |
     |  정규화, content_hash 계산, 공신력 tier 산출, UNIQUE 로 중복 제거
     v
  Silver   MariaDB   테이블 6개, 서비스 화면과 수집 현황 대시보드가 직접 조회
     |
     +--> Gold   dbt + DuckDB   집계 모델 3개, 테스트 21종이 배포 게이트
     |
     +--> mart_history (Iceberg)   UPDATE, UPSERT 로 사라질 이전 값을 보존
                                   변경분은 실행마다, 전체 스냅샷은 1일 1회


행동 로그, 메달리온 2      Iceberg on GCS 의 behavior 네임스페이스

  GA4 -> BigQuery 일별 내보내기
     |
     v
  Bronze   ga4_events        BigQuery 가 제공한 중첩, 반복 필드를 그대로 적재
     |
     |  Flatten (ARRAY, STRUCT 해제), 중복 이벤트 제거
     v
  Silver   ga4_events_flat   1행 1이벤트, 1열 1값
     |
     |  전량 재구축 (신규, 재방문 판정은 전체 이력이 필요한 비가산 지표)
     v
  Gold     팩트 3개 (카드 클릭, 세션, 일별 사용자 활동), 디멘션 2개 (날짜, 사용자)
```
