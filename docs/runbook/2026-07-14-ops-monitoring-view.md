...
- **"이력 없음" 표시 조건** — 섹션 ① · ② · ③ · ⑤ 는 각 원천이 빈 경우 (콜드 스타트 · 아직 이력이 없는 소스) 표 대신 "이력 없음" 문구로 대체된다 (`ops.html.j2` 의 `{% else %}` 가드).
  섹션 ④ (tier 분포) 는 `articles` 자체 집계라 이 가드가 없다 — 기사 0건이면 분모 0 방지용 `or 1` 가드만 적용된다.
- **콜드 스타트 화면** — `pipeline_runs` · `source_freshness` 모두 0행일 때 KPI 5종은 `—`, 번역 · 분류 대기만 실수 (articles 직접 카운트라 이력에 안 걸림), 섹션 ①②③⑤ 는 "이력 없음", 섹션 ④ 는 있는 그대로 (0건이면 0% 로 렌더).
