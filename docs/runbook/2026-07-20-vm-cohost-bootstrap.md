...
#### 6.1.3. 여러 줄의 배포를 한 사람이 태울 때 (2026-08-23 추가)

병렬 세션 배치에서 배포 주체를 한 줄로 몰면 **줄마다 하나씩, 사이에 정기 회차를 두는 순서**를 지시가 정한다.
회차를 사이에 두는 이유는 **한 회차의 결과를 어느 줄의 몫으로 읽을지 가르기 위해서**다.
2026-08-23 배치에서 그 순서를 태우며 넷을 배웠다.

**① 머지가 한꺼번에 되면 `git pull` 은 나눌 수 없다.**

사용자가 네 PR 을 한 번에 머지하면 `main` 에 넷이 다 있고, `git pull --ff-only` 는 그 지점까지 통째로 올린다.
줄 하나만 올리려고 특정 커밋으로 체크아웃하면 VM 이 분리 HEAD 가 되어 다음 반영이 꼬인다.

→ **관측 축으로 가른다.**
그 배치의 넷은 화면 (서빙 계수) · 수집 (새로 저장되는 저자) · 저널 (알림 줄) 로 축이 갈려 있어, 한 회차에 함께 올라가도 판독이 안 섞였다.
→ **축이 겹치는 줄이 있으면 그때는 회차를 나눠야 하고, 그러려면 머지 순서를 사용자에게 먼저 청한다.**

**② 빠뜨려도 오류가 안 나는 단계는 설치본을 직접 센다.**

유닛 파일이 바뀐 배포는 `git pull` 만으로 `/etc/systemd/system/` 사본이 안 바뀐다 (§5).
`bash infra/systemd/install-units.sh` 를 빠뜨려도 **오류가 안 나고 알림도 안 사라진다** — 그 유닛만 옛 사본으로 조용히 남는다.

```bash
# 저장소 사본과 설치된 사본을 같은 문자열로 센다 (반영 전 0 · 반영 뒤 1)
ssh <호스트> "grep -c CHANNEL_INCIDENT /etc/systemd/system/bullet-in-fail-notify@.service"
ssh <호스트> "grep -c CHANNEL_INCIDENT ~/bullet-in/infra/systemd/bullet-in-fail-notify@.service"
```

**③ 확인선을 둘로 가른다 — 결정적인 것과 조건부인 것.**

배포마다 「반영됐나」 와 「효과를 봤나」 는 다른 질문이고, 뒤쪽은 조건이 맞는 회차에만 답이 나온다.

| 갈래 | 무엇으로 | 언제 |
| --- | --- | --- |
| ① 결정적 | VM 커밋 · 파일 안의 그 줄 · 설치된 유닛 사본 | **회차를 안 기다린다** |
| ② 조건부 | 그 조건이 실제로 발생한 회차 | 며칠 뒤일 수 있다 |

→ **②가 안 나온 것을 「고쳐졌다」 로 적지 않는다.**
같은 배치에서 예외 오류 문자열 수정은 「타임아웃이 난 회차」 에만, 발견 퍼널 계수는 「알림이 발화한 회차」 에만 드러났다.
둘 다 며칠 안 나올 수 있고 **안 나오는 것이 정상**이다.
→ 보고에는 **「①까지 확인 · ②는 조건 대기」** 로 적는다.

**④ 저널이 답할 수 있는 것만 저널로 묻는다.**

「이 회차에 알림이 몇 건 나갔나」 를 저널 `grep` 으로 판정했다가 틀렸다.
그때 `send_alert` 는 **실패에만 로그를 남겼다** (`알림 (webhook 미설정)` · `알림 발송 실패` · `알림 발송 오류`).
성공 발송에는 로그가 없었다.

- 그 시점의 저널이 답할 수 있는 것은 **「발송 실패가 없었다」** 와 **「신선도 알림은 0건이었다」** 둘뿐이었다 (뒤쪽은 그 경로만 `발송 N` 을 따로 찍는다).
- 실제로는 그 회차에 확인요청 채널로 알림 둘이 나가 있었고, 화면과 `pipeline_runs` 로만 보였다.
- **「없음」 을 확인하는 검사는 검사가 헛돌아도 「없음」 을 낸다** — 세션 메모리 `verification-that-silently-passes` 의 재판이다.

**이 구멍은 같은 날 메워졌다** (#328) — 성공 발송에도 `INFO` 한 줄이 남아 이제 `grep '알림 발송:'` 으로 회차별 발송을 셀 수 있다.
판독법은 `2026-07-13-collection-alerts-ops.md` 의 「Discord webhook 설정」 절에 있다.
경위 전체는 알림 축 회차가 따로 남겼다 (`docs/troubleshooting/2026-08-23-we-asked-the-journal-a-question-it-could-not-answer.md` · PR #330).
**남는 규율은 그대로다** — 무엇을 물을지 정했으면 **그 질문에 답할 수 있는 자리인지부터 확인한다.**
