...
## 7. 코드 규칙을 바꿨을 때의 델타 — 코드를 안 고치고 재기 (2026-08-22 추가)

§2 는 **설정**을 바꿨을 때를 다룬다 (등재 · 별칭 · 저장값 정정).
**표시 규칙 자체를 바꿀 때도 코드를 한 줄도 안 고친 채 델타를 잴 수 있다** — 설계 회차에서 처방의 효과를 미리 보고 스펙의 판정 기준을 세울 때 쓴다.

### 7.1. 원자료를 「지금 서빙되는 집합」 으로 먼저 자른다

`articles` 전량과 실제 서빙 대상은 다르다 (fmkorea 무관 글 서빙 제외 등).
그 필터를 다시 구현하지 말고 **배포 산출물에서 대상 목록을 뽑아 낸다.**

```bash
# 산출물에 실린 기사만 남긴다 — 서빙 필터를 재구현하지 않아도 집합이 맞는다
grep -o 'article/[0-9a-f]\{64\}\.html' all.html | sed 's|.*/||;s|\.html||' | sort -u
```

실측 — `articles` 807행 중 산출물에 실린 것은 787건이었고, 이 한정 없이 재면 항목 종수는 맞는데 **건수가 열 종에서 어긋났다.**
종수만 볼 때는 안 드러나고 분포 (몇 건짜리가 몇 종인가) 를 볼 때 드러난다.

### 7.2. 새 규칙을 함수 경로에 끼워 넣는다

서빙 함수를 **감싸서** 새 규칙을 넣고 같은 계산을 다시 돌린다.
저장소 코드는 건드리지 않으므로 설계 회차의 「코드 무수정」 제약을 지킨다.

- 규칙이 쓸 값이 그 함수에 안 들어오면 (예 — 기자 뷰 함수가 기사 행을 안 받음) **호출부에서 그 값을 채워 넣는 얇은 껍질**을 씌운다.
- **원본 함수를 다시 구현하지 않는다** — 손으로 흉내 내면 §3.1 의 인자 함정을 그대로 밟는다.

### 7.3. 절대값을 먼저 산출물과 맞추고 나서 델타를 잰다

**이 순서를 뒤집으면 안 된다.**

```
[재현] 기자 항목 205종 · 소속 라벨 89종 · 첫 화면 30종
[산출물] 205종 · 89종 · 30종
[항목별 대조] 라벨 · 건수 불일치 0종
```

- 종수만 맞춰 놓고 넘어가지 말고 **항목마다 라벨과 건수까지** 대조한다.
- 여기서 0 이 나온 뒤에야 델타를 믿을 수 있다 (§4 의 「전후를 같은 틀린 호출로 재면 오차가 상쇄된다」 가 이 자리다).

### 7.4. 델타는 늘 쪽과 주는 쪽을 함께 센다

이 회차의 처방은 **소속 라벨 89종 → 145종 · 사라진 항목 0 · 새로 생긴 항목 0** 이었다.
「라벨이 늘었다」 만 보고했으면 항목이 갈리거나 합쳐졌는지를 못 봤을 것이다.

**표기나 별칭을 건드리는 처방은 접기와 신설이 함께 일어난다** — 양쪽을 안 세면 순증감이 0 이어도 내용이 바뀐다.

### 7.5. 이 방법이 답하지 못하는 것

- **조작 횟수 · 배치 · 밀도** — 「더보기를 몇 번 눌러야 하나」 는 값이 아니라 화면이다.
목업으로 간다 (`docs/runbook/2026-08-22-choosing-display-options-with-a-real-mockup.md`).
- **저장 시점에 굳는 값** — 공신력 (`articles.tier`) 처럼 수집 때 계산돼 저장되는 값은 렌더 재현으로 안 움직인다.
그 영향을 보려면 **판정 함수를 직접 불러 전후를 비교한다** (이 회차에서 `resolve_tier` 를 전 소스 × 다섯 이름으로 돌려 실제로 달라지는 조합이 하나뿐임을 확인했다).
