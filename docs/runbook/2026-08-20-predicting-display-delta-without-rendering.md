...
### 3.3. 표시 함수 밖에서 예고를 만들지 않는다 (2026-08-22 추가)

§3.1 · §3.2 는 함수를 부르되 입력을 잘못 준 자리다.
**함수를 아예 안 부르거나 그 출력을 덜 읽고 답을 낸 자리가 하루에 두 번 나왔고, 둘 다 값이 그럴듯해서 데이터를 먼저 의심하게 만들었다.**

#### 3.3.1. 설정 조회로 표시 층의 물음에 답했다

「소급이 넣을 이름 `Alex Howell` 이 사이드바 항목을 새로 만드나」 를 사전 조회 한 줄로 답했다.

```python
journalist_directory("config/credibility.yaml").get(norm_alias("Alex Howell"))   # None
```

- **`None` 을 「항목이 없다」 로 읽었는데 그 이름은 이미 12건짜리 항목으로 첫 화면에 앉아 있었다.**
- **사전은 「정식 표기로 접을까 · 등급을 줄까」 를 정하지 항목의 유무를 정하지 않는다** — 미등재 이름도 기사에 저장된 tier 로 항목이 선다.
- 저장값을 한 번 조회했으면 (`authors_json` 에 그 이름이 든 행 12) 그 자리에서 갈렸다.
- **예고가 「항목 +1」 에서 「그 항목의 건수 +2」 로 바뀐다** — 병렬 세션에 이미 보낸 값이라 정정을 따로 보내야 했다.

#### 3.3.2. 함수는 불렀는데 출력의 새 자리를 안 읽었다

기자 축 구현 세션의 관측이다 (그쪽 재현 계수 · 서빙 819건 · 배포 전이라 산출물 계수와는 다른 자).

- 구현이 더보기 단계에 `items` 라는 자리를 새로 만들었는데 계수 스크립트는 옛 자리 (`groups` · `unregistered`) 만 읽고 있었다.
- 그래서 **「기자 항목 216 → 86종 · 사라진 항목 145종 · 새로 생긴 항목 15종」** 이 나왔다.
같은 실행의 맞는 값은 216 → 233종이다.
- **두 수를 함께 적어야 이 함정이 보인다** — 사라진 145종만 적으면 216 − 145 가 86 이 아닌 것이 오타로 읽히고, 도구가 「사라졌다」 와 「모양이 바뀌었다」 를 못 가른 것이 안 드러난다.
- **규칙을 바꾸면 출력의 모양도 바뀐다** — 옛 스크립트를 그대로 쓰면 새 자리가 통째로 안 세어진다.

**검산이 붙어 있다** — 뷰모델의 `total` 이 그 축의 항목 수라, 읽어 모은 수와 대 보면 자리를 빠뜨렸는지 그 자리에서 걸린다.

```python
assert len(names_i_read) == facets["journalists"]["total"]
```

#### 3.3.3. 그래서 규칙 하나

**「화면에 무엇이 보이나」 는 화면을 만드는 함수에만 묻는다.**

- 설정 · 사전 · 저장값은 그 함수의 **입력**이지 답이 아니다.
- 함수를 불렀으면 **그 출력의 모든 자리를 읽었는지** 검산한다.
- 예고를 다른 세션에 보냈으면 **틀렸을 때 정정도 보낸다** — 받은 쪽은 그 값을 자기 기준선에 이미 적어 두었다.
