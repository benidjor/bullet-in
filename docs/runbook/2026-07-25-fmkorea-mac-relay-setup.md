...
### 5.1. 공백이 길어진 뒤의 회차 (2026-09-01 추가)

맥이 오래 꺼져 있다 다시 붙으면 보충 수집이 스스로 검색 범위를 넓힌다.
1 페이지만 읽으면 그 사이 밀려난 글이 2 페이지로 넘어가 다음 회차에도 안 보이기 때문이다
— 지연이 아니라 손실이 된다.

- 접촉 공백이 **12 시간을 넘길 때마다 검색 페이지를 하나 더** 읽고 3 페이지에서 멈춘다.
- 넓히는 회차에만 요청 간격 1.5 초 · 후보 상한 45 · 이미 적재된 제목 배제가 함께 걸린다.
- **정상 주기에는 어댑터 인자가 하나도 안 바뀐다** (공백 12 시간 이하).
정상 주기에서는 이미 적재된 제목을 일부러 배제하지 않는다
— 재수집이 본문 등급을 올리는 경로를 살려 둔다.

넓힌 회차는 로그에 아래 형태의 한 줄을 남기게 돼 있다 (아래는 값을 채워 넣은 예시다).

```
INFO fmkorea 따라잡기 — 공백 27.3h · 검색 3페이지 · 기존 308건 배제
```

**이 경로가 실제로 발동한 것은 아직 못 봤다** (2026-09-01 기준).
정상 운영에서는 12 시간 공백이 잘 안 생기므로 맥을 오래 끈 뒤 첫 회차의 로그에서 위 문구를 찾아 확인한다.

### 5.2. `[공홈]` 이 후보 자리를 먹지 않는지 확인 (2026-09-01 추가)

같은 회차에 `[공홈]` 말머리 글을 **본문을 받기 전에** 버리도록 고쳤다.
고치기 전에는 본문을 받은 뒤에 버려서 후보 한 자리와 fmkorea 요청 한 번을 그때마다 잃었다.

정기 회차 로그에서 다음 두 줄을 함께 본다.

```
INFO fmkorea [공홈] 말머리 drop — 직수집 경로가 커버 url=...
INFO 소스별 후보 계수: {..., "fmkorea": 13}
```

- **drop 줄의 개수만큼 본문 요청이 안 나간 것이다** (2026-09-01 18:02 회차 실측 13 건).
- 같은 회차의 fmkorea 후보 계수가 13 이었으므로, 고치기 전이라면 이 26 건이 상한 15 를 나눠 가졌을 것이다
— 다만 몇 자리를 뺏겼을지는 라운드로빈 순서에 딸리므로 **추정이고 실측이 아니다.**
- 상한 15 에 늘 닿는 것은 아니다.
후보가 15 미만인 회차에서는 자리를 뺏기지 않으므로, 이 수정의 확실한 이득은 **요청 절감** 쪽이다.

관련 = `docs/troubleshooting/2026-09-01-posts-we-throw-away-were-eating-the-candidate-cap.md`.
