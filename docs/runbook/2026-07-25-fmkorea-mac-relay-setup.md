...
- `com.bulletin.fmkorea-tunnel` 은 `KeepAlive` 로 상주하고, 로그는 `/tmp/fmkorea-tunnel.err` 에 쌓인다.
- `com.bulletin.fmkorea-supplement` 은 `RunAtLoad` 로 로드 직후 1 회, 이후 900 초마다 실행을 시도하고, 로그는 `/tmp/fmkorea-supplement.out` · `.err` 에 쌓인다.
...
```bash
# 맥: launchd 언로드
launchctl unload infra/mac-fmkorea-relay/com.bulletin.fmkorea-supplement.plist
launchctl unload infra/mac-fmkorea-relay/com.bulletin.fmkorea-tunnel.plist
```
