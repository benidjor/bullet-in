...
**`restart` 가 배치를 한 번 즉시 실행시킨다** (2026-08-13 실측 · 예상 못 한 동작).
유닛에 `Persistent=true` 가 있어 systemd 가 「놓친 회차」 를 보충 실행하는데, 시각을 옮기면 새 스케줄의 지난 시각 중 마지막 실행보다 뒤인 것이 생겨 그것을 놓친 회차로 판정한다.

- 실측 — 마지막 실행 10:31 UTC · 새 스케줄에 13:30 UTC 가 있고 현재가 14:47 UTC 라, `restart` 직후 배치가 돌았다 (37초 · 검색 10명 · 적재 1 · 검색 실패 0).
- **정기 회차와 가까운 시각에 반영하지 말 것** — 이 실측에서는 정기 회차 15분 전에 fmkorea 접촉이 한 번 더 나갔다 (§7 접촉 예산의 60분 간격보다 좁다).
- 반영은 정기 회차 직후에 하고, `list-timers` 의 `LAST` 가 방금 시각이면 보충 실행이 돈 것이다.
- 한 회차가 앞당겨 돌 뿐 일순 커서는 이어지므로 데이터에 손상은 없다.
