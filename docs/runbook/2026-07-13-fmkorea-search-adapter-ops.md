...
## 저장되는 URL 은 fmkorea 주소가 아니다 (2026-08-04 확인)

`articles.url` 에 들어가는 것은 **fmkorea 게시글 주소가 아니라 원문 언론사 주소**다.
어댑터가 본문에서 원문 URL 을 뽑아 `RawItem(url=orig)` 로 항목을 만들고, 원문 URL 을 못 구한 글은 아예 수집에서 빠진다 (위 로그 해석의 `원문/말머리 해소 실패 — 스킵`).

이 사실이 운영에서 두 가지를 바꾼다.

- **URL 로 다시 접촉하는 도구는 fmkorea 에 접촉하지 않는다.**
`source_id = 'fmkorea'` 인 행을 재수집해도 요청은 해외 언론사로 나간다.
따라서 그런 작업에 430 차단 확인 절차를 붙일 이유가 없다.
- **재접촉 성공률은 생각보다 높다.**
2026-08-04 발행일 복구 실측에서 21건 중 16건이 성공했고 실패는 3건뿐이었다 (Telegraph 403 · sky-sport.ch 403 · besoccer 406).
어댑터 주석의 "26건 중 25건이 406 · 403 · 페이월" 은 **수집 시점에 본문을 받아 오는 경로의 수치**이고, 나중에 URL 하나만 다시 여는 것과는 조건이 다르다.
그 수치를 재접촉 성공률로 옮겨 쓰면 안 된다.
- **나머지 2건은 애초에 기사 URL 이 아니었다** (`docs/troubleshooting/2026-08-05-non-article-urls-stored-as-articles.md`).
...
- **원인** — 짧은 시간에 fetch 를 여러 번 돌리면 430 (429 아님) · 20분+ 지속.
  IP 유형과 무관하게 발생 (2026-07-20 관측: VM 데이터센터 IP 의 무인 회차에서 3키워드 전부 430
  — 직전 수동 회차와 약 1.5시간 간격, 키워드 스킵으로 회차는 정상 완주).
