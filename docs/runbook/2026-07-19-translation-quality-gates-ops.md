...
트랙 ③ · 상세 점검에서 도입한 번역 품질 자동 게이트 3축 + 표기 사전 2종의 운영 절차.
모두 결정적 (LLM 재호출 없음) 후처리 · 검증이라 비용이 없고, 실패 시 재번역 큐 → 폴백으로 수렴한다.
도입 PR: #76 (정방향 환각) · #78 (라운드업 완전성) · #80 (역방향 오역) · glossary 확장 #72 · #81.
...
- **사전 2종**: `config/glossary.yaml` (오표기 → 통용 치환, 저장 직전 적용)
...
- **`재번역 큐 content_hash=… 환각의심=… 단신누락=…`** — 1차 검출. 다음 사이클에 자동 재시도되므로 즉시 조치 불필요.
...
2. 같은 인물의 복수 한글 표기 (래시포드 · 라시포드) 는 각각 등재 — 검출기가 영문 값 기준으로 동일 인물 처리.
...
with e.connect() as c:
    rows = [dict(r) for r in c.execute(text(
        ...
        "FROM articles WHERE title_ko IS NOT NULL")).mappings().all()]
for r in rows:
