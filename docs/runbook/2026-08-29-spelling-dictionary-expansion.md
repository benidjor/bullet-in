...
- **그 취약함은 옆에서 해시를 갈 수 있는 작업 (주소 정정) 이 돌면 실제로 발화한다.**
2026-08-29 에 병렬 세션의 주소 재정규화가 기사 하나를 재번역 경로로 밀어 넣었고,
거기서 사전의 `club_map` 짝 누락이 드러났다.
**병렬로 도는 작업 중 주소 · 원문 제목을 만지는 것이 있는지 먼저 묻는다.**
