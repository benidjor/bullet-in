...
- 두 알림 경로 — 하드 실패 (회차 유닛이 0 이 아닌 코드로 끝나면 systemd `OnFailure` 가 `bullet-in-fail-notify@.service` 를 띄운다) · 소프트 드리프트 (실행은 성공했으나 한 소스가 조용히 0건 = 셀렉터 드리프트 → 소스별 수집량 이상탐지) .
...
- **네 구획으로 나뉜다** — 무슨 일이 있었나 · 평소와 비교 · 지금 어떤 상태인가 · 무엇을 하나.
  앞 둘은 관측이고 뒤 둘은 판단 재료다.
- **소스가 하나면 구획이 각각 embed 필드로 나가고, 여럿이면 소스당 한 필드로 접힌다.**
  구획을 펼친 채로 소스가 일곱 곳 동시에 끊기면 필드 상한 (25) 을 넘겨 **알림이 발송에 실패한다** — 하필 전면 장애 때 알림을 잃는 자리라 접는다.
  접힌 배치에서는 구획 제목이 `**▸ 무슨 일이 있었나**` 처럼 굵은 한 줄로 들어간다.
...
- **「지금 어떤 상태인가」** — `회차는 실패로 끝나지 않았습니다 (success_rate 1)` 이 사실이고, 그 아래 기울인 줄이 뜻풀이다.
  「정상 종료」 라고 안 쓰는 이유는 아래 대응 절에 있다.
- **문안의 서식은 장식이 아니다** — 셋 다 실물 화면에서 고쳐진 자리다.
  **검색 키워드는 중첩 목록 한 줄씩이다** — 한 줄에 몰아 쓰면 디스코드가 들여쓴 이어붙임 줄을 앞 불릿에 흡수해 한 덩어리로 흘린다 ( 2026-08-22 실물 ) .
  **구획 제목은 필드 이름으로 낸다** — 마크다운 굵기 (`**▸ …**`) 와 `###` 헤더를 나란히 쏴 대 본 결과 필드 이름이 이겼다 ( 크기 · 여백까지 디스코드가 그려 준다 · 2026-08-23 실물 ) .
  **구획 사이 여백은 폭 없는 공백 (`​`) 한 줄이다** — 필드 간격 자체는 고정이라 값 끝에 그 줄을 붙여 띄운다.
  값이 없는 구획은 필드가 아예 안 생긴다.
- **발견 퍼널 4단** — `발견 퍼널: 목록 13 → URL 13 → 제목 7 → 키워드 3` 은 각 단을 통과해 **남은 수**다.
  **목록이 0이면 셀렉터가 깨졌다**.
  앞은 멀쩡한데 **키워드만 0이면 원문이 조용하다**.
  종전에는 마지막 단 하나만 기록해 이 둘을 가르지 못했다 ( 스펙 §2.5 · `bbc_sport` 라이브 실측 13 → 13 → 7 → 3 ) .
  html 어댑터만 이 계수를 내놓는다 — `rss` · `x_playwright` · `fmkorea` 는 발견 단계가 달라 그 줄이 빠진다.
- **원인은 여전히 적지 않는다** — 후보 0 이 차단인지 셀렉터 드리프트인지는 알림 시점에 알 수 없다.
  응답 코드와 퍼널 계수는 관측한 사실이라 적고 어댑터 힌트 ( 추정 ) 는 안 붙인다.
...
- **보충 수집** (`collect_fmkorea`) — Mac launchd (`com.bulletin.fmkorea-supplement`) 가 15분마다 ssh 로 실행한다.
  ...
  3시간 접촉 가드가 걸려 있어 이 경로의 실제 접촉은 훨씬 드물다.
- **간격을 잴 때 단위를 섞지 않는다** (2026-09-01 추가).
  스탬프에 찍히는 접촉 시각은 **세 경로가 합쳐진 값**이라, 보충 경로 하나의 3시간 가드보다 촘촘하게 나온다
  — 2026-08-30 ~ 08-31 로그의 접촉 17회는 간격이 평균 2.5시간 · 최대 4시간 3분 · 최소 1시간 28분이었다.
  이 수를 보충 경로의 주기로 읽으면 가드가 안 걸린 것처럼 오해한다.
  **로그 줄 수로 실행 횟수를 세는 것도 안 된다** (건너뛴 회차는 한 줄이고 접촉한 회차는 여러 줄을 남긴다).
