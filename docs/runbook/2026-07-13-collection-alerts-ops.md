...
- **문안의 마크다운은 장식이 아니다** — 구획 제목은 굵게, 검색 키워드는 중첩 목록 한 줄씩, 뜻풀이는 기울임이다.
  **키워드를 한 줄에 몰아 쓰면 디스코드가 들여쓴 이어붙임 줄을 앞 불릿에 흡수해 한 덩어리로 흘린다** ( 2026-08-22 실물 화면에서 그렇게 나갔다 ) .
  구획 제목도 굵기가 없으면 바로 아래 불릿과 같은 무게로 그려져 층이 안 보인다.
