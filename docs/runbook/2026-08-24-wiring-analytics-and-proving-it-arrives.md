...
**여기서 확인할 것은 「이벤트가 도착하는가」 하나다.**
원본을 따로 받을 계획이면 맞춤 측정기준을 화면에 등록할 필요가 없다.

**매개변수는 표준 보고서에 안 나오지만 실시간 개요에서는 볼 수 있다 (2026-09-18 정정).**
「이벤트 이름별 이벤트 수」 에서 이벤트를 누르면 매개변수 키 목록이 나온다 — 30분 안에 그 이벤트가 있어야 한다.
그전에 이 절은 「매개변수는 이 화면에 안 나온다」 고 적고 있었는데, 실제로 찍어 보니 나왔다 (`docs/runbook/2026-09-19-capturing-ga4-and-bigquery-screens-with-the-chrome-extension.md` §3).

아래는 2026-09-18 에 찍은 화면이다.
계정 · 프로젝트 식별자는 가렸다.

![GA4 이벤트 보고서](../assets/ga4-events-table.png)

> GA4 보고서 → 이벤트, 2026-08-29 부터 09-17 까지다.
> 직접 정의한 이벤트 넷이 모두 도착해 있다 — `bi_entry` 3,769 · `bi_card_click` 628 · `bi_filter_apply` 238 · `bi_origin_exit` 15.

![GA4 실시간 (bi_card_click 의 매개변수)](../assets/ga4-realtime-card-hash-param.png)

> 실시간 개요에서 `bi_card_click` 을 펼친 화면이다.
> 매개변수 `card_hash` 가 기사의 `content_hash` 와 같아 클릭 로그를 Silver 의 기사 행과 조인할 수 있다.
> 익명 식별자 `bi_cid` 와 클라이언트 시각 `bi_ts` 는 모든 이벤트에 실린다.
...
![GA4 관리 → BigQuery 링크](../assets/ga4-bigquery-link.png)

> 연결을 마친 화면이다 (2026-09-18 캡처).
> 내보내기 유형 「매일」 · 데이터 세트 위치 서울 (asia-northeast3) · 연결일 2026-08-24.
...
![BigQuery 데이터셋의 일별 표](../assets/bigquery-events-tables.png)

> 데이터셋 `analytics_<속성ID>` 의 일별 표 `events_YYYYMMDD` 다 (2026-09-18 캡처 · 08-24 와 08-28 부터 09-17 까지 22개).
> `warehouse_load` 는 이 목록에서 아직 적재하지 않은 날짜만 골라 처리한다.
> 당일이 끝나면 사라지는 `events_intraday_*` 는 읽지 않는다.
