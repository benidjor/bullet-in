...
`catchup=False` · `max_active_runs=1` · `dagrun_timeout=30분` 은 지금까지의 systemd 타이머 성질 (밀린 회차는 최근 구간 한 번 · 이중 실행 금지) 을 그대로 옮긴 것이다.
...
`pending` 이 참인 채로 다음 회차로 넘어가고, 다음 회차의 `judge` 가 그 `pending` 을 이어 판정한다 — 판정은 없고 회차가 조용히 사라지지 않게 하는 것이 이 콜백의 목적이다.
...
- 손 실행 둘 (`scheduled__…06:07:47` 앞 · `manual__…06:07:47` 뒤): 소요 3분 20초 · 3분 16초, 전부 태스크 여덟 success — systemd 회차와 같은 급.
...
태스크 0 의 SQLite 검증과 최종 코드 리뷰 (3.3.1 을 깔아 소스 대조) 를 지나 실물 설치 · 실행에서야 드러난 결함 여섯 (구멍 셋은 실물 전에 잡혔다) 이다.
...
| 자리 | 증상 | 처방 |
| --- | --- | --- |
| 설치 `db migrate` | `ModuleNotFoundError: asyncpg` | `apache-airflow[postgres]` |
| `judge` 태스크 셸 | Task SDK 가 DB 접속을 막아 둠 | CLI 앞에 `airflow.env` 재로딩 |
| 감시 `dags list-runs` | dag_id 가 위치 인자인데 `-d` 로 넘김 → 영구 오탐 | 인자 정정 + 실패 경로 경고 로그 |
| 감시 `jobs check --allow-multiple` | 3.3.1 은 `--limit` 도 요구 → rc=1 | 플래그 제거 |
| 감시 `dags list-runs -o json` | structlog 경고가 stdout 앞줄에 섞여 JSON 파싱 실패 | 첫 `[`/`{` 줄부터 읽기 + `PYTHONWARNINGS=ignore` |
| `deploy_site` 의 `.sh` 끝 | BashOperator 가 Jinja 템플릿 파일로 읽으려 함 | 명령 끝에 공백 하나 |
