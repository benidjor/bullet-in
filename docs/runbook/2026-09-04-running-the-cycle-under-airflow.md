...
### 6.5. 첫 24시간 (2026-09-04 18:00 에서 09-05 15:00 KST · 정규 8회)

전환 (15:51) 뒤 첫 정규 실행부터 여덟 회를 2026-09-05 22:50 KST 에 대조했다.
잣대는 넷이다.
실행 상태 · 소요 (`dagrun_timeout` 30분 대비) · 감시 발송 · 태스크 로그의 경고 · 오류 줄이다.

| 회차 (KST) | run_id (UTC) | 상태 | 소요 | advance | judge |
| --- | --- | --- | --- | --- | --- |
| 09-04 18:00 | `scheduled__2026-09-04T09:00` | success | 3.6분 | `d9cd66d` → `eb02b51` (#465) | 반영 완료 |
| 09-04 21:00 | `scheduled__2026-09-04T12:00` | success | 4.0분 | `eb02b51` → `c3924cb` (#466) | 반영 완료 |
| 09-05 00:00 | `scheduled__2026-09-04T15:00` | success | 4.3분 | 없음 | 판정 대기 없음 |
| 09-05 03:00 | `scheduled__2026-09-04T18:00` | success | 3.5분 | `c3924cb` → `730dc17` (#467) | 반영 완료 |
| 09-05 06:00 | `scheduled__2026-09-04T21:00` | success | 3.5분 | `730dc17` → `a498e93` (#468) | 반영 완료 |
| 09-05 09:00 | `scheduled__2026-09-05T00:00` | success | 6.0분 | `a498e93` → `835b08a` (#469 · #470) | 반영 완료 |
| 09-05 12:00 | `scheduled__2026-09-05T03:00` | success | 4.2분 | 없음 | 판정 대기 없음 |
| 09-05 15:00 | `scheduled__2026-09-05T06:00` | success | 3.9분 | 없음 | 판정 대기 없음 |

- **실패 0 · 재시도 0** — 여덟 회 전부 태스크 여덟이 첫 시도에 success 다 (`attempt=2.log` 가 하나도 없다).
- **소요** — 3.5분에서 6.0분 · `dagrun_timeout` 30분의 5분의 1 아래다.
  6.0분짜리 09:00 회차는 PR 1 (#469) 의 gold 표 다섯을 처음 만든 `warehouse_load` 와 두 PR 을 한 번에 받은 `advance` 가 겹친 회차다.
- **감시 발송** — 전환 뒤 0건이다.
  09-04 15:00 이후 저널에 남은 발송 1건은 15:40 의 리허설 2 (감시 스크립트가 고쳐지기 전의 오경보 · §6.3) 이고 첫 정규 실행 (18:00) 보다 앞이다.
- **로그의 경고 · 오류** — 태스크 로그 여덟 회 × 여덟 태스크에서 `warning` · `error` 수준 줄이 0 이다.
  `warehouse_load` 의 stdout 에는 pyiceberg 의 `Failed to delete metadata file` 이 회차마다 2에서 8줄 남는데 (09:00 회차가 8줄 · gold 표가 다섯으로 늘어난 첫 회차), 덮어쓴 뒤 옛 메타데이터 파일을 지우려다 GCS 권한에서 막히는 경고라 적재 결과와 무관하다 (`docs/runbook/2026-09-05-reproducing-a-baseline-from-a-pr-branch-on-the-vm.md` §4 · 안건 등재 여부는 미정).
- **배포 자동화** — 머지가 있던 다섯 회차에서 `advance` 가 `origin/main` 을 받고 `judge` 가 「반영 완료」 를 냈고, 없던 세 회차는 「판정 대기 없음」 이다.
