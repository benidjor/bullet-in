...
- fmkorea 는 430 rate-limit 이력, afcstuff 는 Playwright 세션 ( 쿠키 주입 ) 이 세트당 2회 발생.
...
| 3 | 2026-07-14 18:06 | 102.17 | 41.62 | 59.3 | 유효 — 순차 fmkorea 정상 21.7s, 병렬 패스만 430 ( 측정 중립, §3 ) |

- 중앙값: **56.5%** ( 유효 3회 = 54.9 · 56.5 · 59.3 )
- 최장 소스: **x_afcstuff** ( Playwright, 순차 40.4 ~ 42.9s — 3회 모두 병렬 시간의 하한 )
- ~70% 미달 사유: 병렬 시간 하한 = 최장 소스 1개.
  순차 합 79 ~ 102s 에서 x_afcstuff ~42s 가 지배해 이론 상한이 ~54 ~ 59% — 70% 는 현 소스 구성에서 구조적으로 불가.
  README §4 목표를 실측 기반 ( ≥ 55%↓ ) 으로 재조정 ( spec §3.4 방침 ).
