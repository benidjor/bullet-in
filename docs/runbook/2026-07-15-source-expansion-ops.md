...
- 한 소스만 끄는 방법은 **그 소스로 적재된 기사가 있는지에 따라 갈린다** — 아래 §6.1.

### 6.1 소스 수집을 멈출 때 (2026-08-15 개정)

**`enabled: false` 는 「수집만 중단」 이 아니다.**
같은 플래그를 수집 (`adapters/factory.build_adapters`) 과 **표시** (`score.load_sources`) 가 함께 본다.
소스가 표시 설정에서 사라지면 이미 적재된 기사가 세 군데서 깨진다.

| 잃는 것 | 왜 |
| --- | --- |
| 언론사 이름 | `articles.outlet` 이 비어 있으면 소스 설정에서 가져오는데 그것이 사라져 소스 이름 (`goal`) 이 그대로 칩에 나온다 |
| 공신력 | `_outlet_tier` 가 소스 설정의 `tier` 로 폴백하는데 그것이 없어져 미등재로 떨어진다 |
| **본문** | 상세 페이지 본문 노출 범위가 소스 설정의 `serving` 값이라 **발췌로 준다** |

goal 을 끄려다 실측한 값이다 — 저장 기사 171건에 **481장이 바뀌었다.**

**`football_london` 선례를 옮겨 쓰면 안 된다.**
그 소스가 `enabled: false` 로 안전했던 것은 **적재된 기사가 0건**이어서다.
선례를 쓸 때는 그 선례가 통했던 조건이 지금도 성립하는지 본다.

**적재된 기사가 있으면 `collect: false` 를 쓴다.**

```yaml
- source_id: goal
  collect: false        # 수집만 멈춘다 — 표시 설정은 남는다
  freshness_hours: 0    # 수집 중단 소스는 신선도 감시에서 뺀다
```

- `load_sources` 는 `collect` 를 안 보므로 표시가 그대로다 (화면 변화 0장을 렌더로 확인).
- **`freshness_hours: 0` 을 빠뜨리면 안 된다** — 표시에 남는다는 바로 그 이유로 신선도 감시 대상에도 남는다.
수집이 멈추면 마지막 수집 시각이 안 움직여 **48시간마다 영원히 알린다.**
- 배포 직후 회차에 **후보 절벽 알림이 한 번 온다** (전이만 잡으므로 그 뒤로는 안 온다) — 사고가 아니므로 미리 알린다.
- 수집량 이상탐지는 소스에 따라 갈린다 — 회차 평균이 `min_baseline` (3.0) 미만이면 안 걸린다 (goal 은 0.75 로 안 걸렸다).

알림 셋의 판정은 `docs/superpowers/specs/2026-08-14-slo5-freshness-alert-blind-spot-design.md` §5 가 정본이다.
