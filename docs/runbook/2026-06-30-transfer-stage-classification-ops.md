...
- **429 동작**: Gemini API 는 분당 *요청 수* 한도다. 429 를 만나면 그 회차는 **즉시 중단** 하고 남은 배치는 다음 실행에 누적된다 (멱등). 한 번에 다 안 되면 같은 명령을 다시 돌리면 된다.
...
프롬프트가 기준을 적지 않던 동안 모델이 타 구단 기사에서 그 이적 자체의 영입 · 방출로 답해, 실측 38건이 아스날과 무관한 이적에 방향을 달고 있으며, 프롬프트 정정 이후 표적 재분류로 정리한다 (절차는 계획서 `docs/superpowers/plans/2026-08-03-serve-player-pages-impl.md` Task 5).
