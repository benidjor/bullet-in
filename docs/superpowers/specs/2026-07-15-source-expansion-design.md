...
- **fetch 실패 격리는 기존 것으로 충분** — `ingest.py` 의 소스별 try/except 가 goal 슬러그 재변경 404 · API 장애 · 셀렉터 드리프트를 해당 소스만 errors 로 격리.
  ...
  → 신규 에러 처리 코드 없음.
- **SLO-5 사각 ( 최종 리뷰 반영 )** — 워터마크는 `MAX(fetched_at)` 기반이라 한 번도 수집된 적 없는 소스에는 없다.
  키 부재로 처음부터 스킵되는 guardian 은 SLO-5 알림 대상 밖 ( 회차별 WARNING 로그만 ).
  최소 1회 수집 이후의 키 소실은 48h 임계로 정상 포착.
  → 대응: 배포 환경 ( Airflow ) 의 GUARDIAN_API_KEY 존재 + 첫 운영 회차에서 guardian 이 source_counts 에 나타나는지 확인을 배포 체크로 둔다.
