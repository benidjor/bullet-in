...
### 9.2. 붙어서 쟀다 — 표본 15건에서 부분 유실 0

저장된 저자가 정확히 한 명인 행에서 소스별로 뽑아 다시 열었다 (사용자 승인 2차 · 각 1회 · 전건 200).

| 소스 | 표본 | 구조화 저자가 저장값과 일치 | 화면 바이라인이 한 명 |
| --- | --- | --- | --- |
| bbc_sport | 5 | 5 | 5 |
| skysports | 4 | 4 | 4 |
| goal | 2 | 2 | 2 |
| fmkorea 원문 (marca · The Sun · sport.es · BBC) | 4 | 4 | 4 |
| **합계** | **15** | **15** | **15** |

**어긋난 건이 하나도 없다.**
화면 표기는 매체마다 다른데 (`By Sami Mokbel Senior football correspondent` · `Sam Blitz Football Journalist @SamBIitz` · `Magdy Obaid Aug 15, 2026 04:52+09:00`) 사람 수는 전부 한 명이었다.

**표본이 아니라 구조가 근거다.**
`_walk_authors` 는 JSON-LD 트리를 재귀로 훑어 찾은 `author` 를 **전부** 담는다.
그래서 부분 유실이 나오려면 페이지가 화면에는 두 사람을 띄우면서 구조화 정보에는 한 사람만 적어야 한다.
이번 조사 전체에서 화면과 구조화 정보가 어긋난 페이지는 **BBC 라이브 포스트 하나뿐**이었고 그것은 아예 저자를 안 싣는 쪽이다.

**표본의 치우침은 적어 둔다.**
최근 수집분부터 골라서 지금 코드가 만든 행에 몰려 있다.
공저 분리 규칙이 들어오기 전 (#282 · 2026-08-14) 에 적재된 행은 이 표본에 거의 없다 — 다만 그때의 결함은 여러 이름이 한 값에 뭉치는 모양이었고, 지금 그런 값이 **0건**임은 전수로 확인했다 (§9.1).
