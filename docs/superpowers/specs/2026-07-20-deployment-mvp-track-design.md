...
#### 2.3.1. 개정 (2026-07-20) — 차등 서빙 폐기, 전 소스 전문 서빙

원안의 차등 서빙을 폐기하고 언론사 6종 (bbc_sport · bbc_gossip · skysports · guardian · goal · football_london) 의 serving 값을 excerpt → full 로 되돌린다.

- 번복 사유: 발췌 서빙이 Tier 2-a 에서 만든 본문 번역 UX (전문 번역 · 문단 사이 이미지) 를 언론사 소스 전체에서 후퇴시켰고, 서비스의 핵심 가치 (흩어진 현지 언론 기사를 한국어 전문으로 한곳에서 읽기) 가 발췌로는 성립하지 않는다고 판단함.
- 수용 리스크: 원안이 기록한 2차적저작물 리스크 (전문 번역은 출처 표기 · 링크 · 비영리로도 공개 게시가 면책되지 않음) 를 인지한 상태로 수용하는 결정 — 발췌의 "정당한 범위" 인용 논리를 포기하고, 문제 제기 시 해당 소스 단위로 excerpt 복귀 (config 1줄) 또는 기사 삭제로 대응한다.
- 예외 잔존: fmkorea 경로 ③ (퍼가기 금지 + 페이월, PR #85) 은 서빙 정책이 아니라 수집 단계에서 본문을 저장하지 않는 헤드라인-온리라, 전문 서빙 통일 후에도 보여줄 본문이 없는 물리적 한계로 잔존한다.
- 구현 범위: config serving 값 전환이 전부 — body_ko (전문 번역) 는 mart 에 그대로 있어 재번역 불필요, 재렌더만. excerpt 렌더 경로 (serving_mode 기본값 · excerpt_paras · 템플릿 분기) 는 config 미지정 소스의 안전 기본값으로 유지한다.
...
### 2.5. 축 ③ Gemini 비용 — Tier 1 선불 · 월 실측 약 ₩700

작성 당시 이 절은 무료 티어를 전제하고 결제가 불필요하다고 적었으나 사실이 아니었다.
운영 키가 물린 AI Studio `bullet-in` 프로젝트는 Tier 1 · 선불이고 실제로 과금되고 있다 (2026-07-21 정정 · `CLAUDE.md` 함정 절과 동일 사실).

- 실측 (2026-07-20): mart 205건 · 일평균 신규 19건 · 평균 원문 2.8천자.
- 건당 호출 2~3회 (번역 + 3줄 요약 + 게이트) 로 일 40~60 요청 — 월 실측 약 ₩700 (₩676.74).
- 분당 속도 제약은 기존 설계 (429 식별 시 회차 중단 · 하루 4회 멱등 누적) 가 그대로 흡수.
등급 확인 위치는 AI Studio 프로젝트 화면이다.
- VM 이전으로 달라지는 조건 없음.
