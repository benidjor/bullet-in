...
- **아스날 net 은 title_content 유지** — `아스날` 키워드는 제목뿐 아니라 본문까지 검색한다 (제목 필터 없음) .
  근거 — 아스날과 이적 링크가 있던 선수가 타팀으로 이적하는 기사 (예 : 몽가 맨시티행) 는 제목에 아스날이 없어도 아스날 팬에게 유의미하므로 본문 매칭으로 수용한다.
  트레이드오프 — 본문에만 아스날이 스친 일반 · 타팀 글 일부가 유입될 수 있으나, 관련 이적 소식을 놓치지 않는 것을 우선한다 (향후 team 분류로 정제 여지) .
- **키워드 공평 선택** — `max_posts` 는 union 총량에 적용하되, 키워드를 라운드로빈으로 뽑아 채운다.
  근거 — 순차로 채우면 넓은 `아스날` net 이 상한을 다 먹어 뒤의 기자 net (de roche · 온스테인) 이 굶으므로, 각 net 이 공평히 대표되도록 라운드로빈한다.
...
- **검색 키워드 3종** — `아스날` (전 아웃렛 아스날 넓은 그물) · `"de roche"` (정확구문) · `온스테인` .
  각 키워드로 `search.php?mid=<board>&search_keyword=<kw>&search_target=title_content` 를 GET 한다.
  키워드 목록 · 검색 URL 템플릿은 config 로 뺀다 (`config/sources.yaml` 의 fmkorea `config`) .
