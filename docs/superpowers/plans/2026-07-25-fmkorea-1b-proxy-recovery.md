...
정기 회차는 맥이 켜져 있을 때 터널을 타고, 맥 깨어남 시 보충 수집 스크립트가 마지막 성공 3시간 초과분만 채운다.
...
- 중복 가드 기준 시각은 fmkorea `MAX(fetched_at)` (DB) · 임계 3시간 · UTC 비교.
...
```bash
git add src/bullet_in/adapters/fmkorea.py src/bullet_in/adapters/factory.py tests/test_fmkorea_adapter.py
git commit -m "feat(collect): fmkorea 어댑터 proxy 주입 (FMKOREA_PROXY 환경변수)"
```

---

### Task 2: 보충 수집 중복 가드 함수
...
**Interfaces:**
- Consumes: 없음.
- Produces: `should_supplement(last_fetched: datetime | None, now: datetime, gap_hours: float = 3.0) -> bool` — 마지막 성공에서 `gap_hours` 이상 지났거나 기록이 없으면 True.
...
def should_supplement(last_fetched: datetime | None, now: datetime,
                      gap_hours: float = GAP_HOURS) -> bool:
    """fmkorea 마지막 성공 (fetched_at) 에서 gap_hours 이상 지났으면 보충 수집.
    기록이 없으면 (기사 0건) True. now · last_fetched 는 같은 시계 (UTC) 여야 한다."""
    if last_fetched is None:
        return True
    return now - last_fetched >= timedelta(hours=gap_hours)
```
...
git commit -m "feat(collect): fmkorea 보충 수집 중복 가드 (마지막 성공 3h)"
```
...
**Interfaces:**
- Consumes: `should_supplement` (Task 2) · `FmkoreaAdapter(proxy=...)` (Task 1) · `to_articles` · `MartStore` · `RawStore`.
- Produces: `build_fmkorea_adapter(cfg: dict, proxy: str | None) -> FmkoreaAdapter` · `async main(force: bool = False)` — 가드 통과 시 fetch → 적재 (번역 · 렌더 없음).
...
#!/usr/bin/env bash
set -euo pipefail
ssh -i /Users/aryijq/.ssh/seoulnow_deploy ubuntu@155.248.164.17 bash -lc '
  cd /home/ubuntu/bullet-in
  set -a; source .env; set +a
  export FMKOREA_PROXY="socks5://127.0.0.1:1080"
  uv run python -m bullet_in.collect_fmkorea
'
```
