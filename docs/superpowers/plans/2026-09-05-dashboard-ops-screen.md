...
## 실행 중 정정 (2026-09-05 저녁)

계획서 코드를 그대로 옮긴 뒤 리뷰 · 실측과 어긋난 자리를 고쳤다.
코드 블록 자체는 고치지 않았으므로 아래가 우선한다.
판정의 근거는 스펙 (§3.3 인사이트는 값에서만) 이 계획서보다 우선한다는 규칙이다.

| 태스크 | 계획서 | 실제 (고친 것) | 이유 |
| --- | --- | --- | --- |
| 3 | 테스트 16 | 브리프의 테스트 파일은 17 이고 고침 뒤 20 | 계획서의 세기가 틀렸다 |
| 3 | SLO 인사이트 부연 「게이트가 막히면 두 행이 ✕ 로 바뀌고 배포가 멈춘다」 | 제거 · 본문에 게이트 시각 | 값에서 안 나오는 문장이고 픽스처 (not_null 실패 2행인데 SLO-4 ✓) 와 모순 (판정 A) |
| 3 | 「8회에 못 미친 날」 이 오늘을 셈 | 오늘 (부분 하루) 제외 · 「회차가 있던 날 가운데 … (오늘 제외)」 | 매일 7/8 시간 동안 하루가 더 세어진다 (판정 B) |
| 3 | Run Duration p50 · 밴드를 모든 날에 그림 | 회차가 있던 날만 (`run_days`) | 회차 없는 날이 `_pctile([])` = 0 으로 0초 골로 보였다 · `line_chart` 는 빈 칸을 못 받는다 (판정 C) |
| 3 | 등급 히트맵 분모가 전체 행 | 등급이 `TIER_ORDER` 에 있는 행만 (`tier_tot`) · 단계 · 식별률은 전체 | 등급 없는 행이 열 합을 100 아래로 내렸다 · 운영 NULL 등급은 0 (판정 D) |
| 3 | `if tot:` · 임계 인사이트 무조건 | 데이터 있는 주 가드 · `max(default=None)` · 임계가 서로 다를 때만 | 빈 시퀀스 `max()` · 거짓 문장 |
| 4c | 전체 1,767 (`--co` 1,769) | 1,772 passed · 1 skipped (Airflow 미설치 skip) | 테스트 셋 추가 |
| 6 | SVG 30 이상 | 운영 실측 29 (로컬 32) | 회차 없는 날 · 빈 소스가 없는 운영 데이터에서 차트 수가 다르다 · 경계값을 「25 이상」 으로 읽는다 |
| 6 | 소스 이름은 설정 `display_name` | 비활성 소스 (`football_london` · `enabled: false`) 는 `load_sources` 가 걸러 id 로 보인다 | 판정 9 는 활성 소스에만 맞았다 · 고칠지는 최종 리뷰 · PR §5 |

VM 재현 (2026-09-05 18:28 KST · 임시 클론 · 회차 정각 밖) = 절 10 · SVG 29 · 타일 신규 2 · 중복률 98% · 성공률 99.6% · p50 125초 (fetch 85) · 끊긴 소스 0 · 회차 391 (에러 20) · SLO 여섯 전부 충족 · 게이트 09-05 09:02 UTC.
목업 (09-04 19:24) 의 신규 1 · 98% · 99.2% · 127초 · 0 · 384 와 같은 자릿수다.
