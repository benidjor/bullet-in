...
**진단 정정 (2026-07-24 · VM 라이브 실측 — 위 전제 두 개가 낡음)**:

- **goal 은 결함 아님** — 2026-07 에 html 어댑터로 전환돼 (playwright_news 는 미사용 코드) `extract_published_at` 로 발행 시각을 정상 수집 중.
"수집 시각 저장" 관찰은 07-20 이전 레거시 16행에 대한 것이었다.
- **재수집 불필요** — 진짜 결함은 published_at 은 정확한데 precision 라벨만 안 붙이는 어댑터 3곳 (x_afcstuff 62 · guardian 5 · arsenal_official 5 = 72행).
raw_payload 에 원문 시각이 남아 있어 백필로 해소되고 X 쿠키 · fmkorea 접촉이 없다.
세 부류 구분: `docs/troubleshooting/2026-07-24-published-precision-null-three-kinds.md`.
- **arsenal_official 커버리지 기아 신규 발견** — 공홈 오피셜 (Tzolis) 이 통째 누락 (taxonomy 서버 필터 무시 + 전 카테고리 50건 창).
B4 보다 심각해 별도 트랙으로 선행하고, arsenal precision 수정은 그 트랙에 흡수한다.
상세: `docs/troubleshooting/2026-07-24-arsenal-official-filter-starvation.md`.
- **레거시 174행 (html 5개 소스 + fmkorea)** 은 URL 재파싱 백필만 가능 (수율 불확실) — 커버리지 감사 결과를 보고 투자 여부 별도 판단.
- **권장 순서 재정렬** — 전 소스 커버리지 감사 (읽기 전용) → arsenal 복구 → B4 잔여 (afcstuff · guardian) → B3.
