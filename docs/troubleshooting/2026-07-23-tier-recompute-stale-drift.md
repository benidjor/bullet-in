...
## 코드가 판정 함수를 바꾼 회차 (2026-08-31 추가)

**위 절차는 「설정만 바뀐다」 를 전제한다.**
판정 함수 자체를 고친 회차에서는 old 쪽을 **옛 설정 + 현재 코드**로 재면 코드 변경분이
「설정 델타」 로 잘못 집계된다.

- **옛 코드의 동작을 인자로 재현할 수 있으면 그것을 쓴다.**
  실물 — fmkorea 갈래가 확정 저자를 안 보던 시절의 동작은 **현재 코드에 `journalist=None`** 이다.
- 재현이 안 되면 옛 판정 로직을 소급 스크립트 안에 짧게 복제한다
  (실물 — `_TITLE_SCAN_MIN_ALIAS` 도입 회차는 그 상수를 0 으로 두어 재현했다).
- **소급 스크립트에 「옛 동작」 스위치를 인자로 둔다** — 회차마다 값이 달라지므로 하드코딩하면
  다음 회차가 조용히 틀린 델타를 낸다.
