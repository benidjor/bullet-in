...
아스날 FC 뉴스 수집 파이프라인.
다중 소스 (RSS · HTML · Playwright · X) 를 asyncio 로 병렬 수집해
MongoDB (raw · bronze) 와 MariaDB (mart · silver · `content_hash` · URL `UNIQUE` dedup) 에 넣고,
Gemini 로 번역 · 요약한 뒤 dbt 품질 게이트 (DuckDB · gold) 를 통과하면 정적 HTML 로 서빙한다.

**스케줄은 둘이다.**

- **회차와 웨어하우스 적재** — VM 의 Airflow DAG `bullet_in_cycle` (3시간 간격 · 태스크 여덟)
- **워치리스트 · 백업 · 유지보수** — systemd 타이머

**배포는 회차가 한다.**

첫 태스크가 `bullet_in.deploy advance` 로 `origin/main` 을 내려받고,
끝 태스크가 `bullet_in.deploy judge --from-airflow` 로 반영을 판정한다.
**세션이 VM 에서 `git pull` 을 하지 않는다.**

- 설계 — `docs/superpowers/specs/2026-09-03-deploy-automation-design.md` ·
  `docs/superpowers/specs/2026-09-04-airflow-migration-design.md`
- Airflow 화면 · 손 시작 · 되돌리기 — `docs/runbook/2026-09-04-running-the-cycle-under-airflow.md`
